class TipoPagosController < ApplicationController
  ...
  params[:tipo_pago][:creado_por]      = session[:user_id]
  params[:tipo_pago][:actualizado_por] = session[:user_id]
  ...
  private
    ...
    def tipo_pago_params
      params.require(:tipo_pago).permit(:Nombre, :Moneda, :Tipo_Cambio, :Porcentaje_Comision, :Destino, :Estado, :creado_por, :actualizado_por, :creado, :actualizado)
    end
end
