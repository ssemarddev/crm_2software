class OpcionsController < ApplicationController
  ...
  params[:opcion][:creado_por]      = session[:user_id]
  params[:opcion][:actualizado_por] = session[:user_id]
  ...
  private
    ...
    def opcion_params
      params.require(:opcion).permit(:Nombre, :LetraConjunto, :Conjunto, :Estado, :creado_por, :actualizado_por, :creado, :actualizado)
    end
end
